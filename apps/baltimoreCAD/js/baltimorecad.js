...
function plotCrimes() {
  ...
  // Only get back things we can plot
  whereStatement += " and location_1 is not null";
  ...
  socrata.request(socrataRequest, function(response) {
    response.forEach(function(crime) {
      if(crime.location_1 != undefined) {
        // Create a new marker and put it on the map
        var crimeLatLng = new google.maps.LatLng(
            crime.location_1.coordinates[1],
            crime.location_1.coordinates[0]
            );
        ...
        console.log("No plottable location found: " + crime);
      }
    });
