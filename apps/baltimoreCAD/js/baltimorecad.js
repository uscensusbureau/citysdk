...
var sdk, census, socrata, map, geocoder, infowindow;
var map;
var boundaries = [];
var variables = [];
var crimeMarkers = [];
...
function updateBoundaries() {
    ...
    //Construct our request object - lat/lng for Baltimore, default to place level with no variables
    var request = {
        lat: 39.2833,
        lng: -76.6167,
        level: "place",
...
function plotCrimes() {
    ...
    var whereStatement = "crimedate>'" + getTime("start") + "' and crimedate<'" + getTime("end") + "'";

    //Grab the type of crime the user is interested in
    var type_selection = $('#description').find(":selected").val();

    //Let's customize our where statement
    if(type_selection != 'all') {
        whereStatement += " and description='" + type_selection + "'"
    }

    //Grab the arrest result the user is interested in
    
    // Removing from Chicago example
    ...
    var socrataRequest = {
        url: socrataURL,
        dataset: crimeDataset,
        select: "crimedate,crimecode,location,description,weapon,district,neighborhood",
    ...
    socrata.request(socrataRequest, function(response) {
        response.forEach(function(crime) {
            ...
            var content_string = "<strong>Crime Date</strong>: " + formatTime(crime.crimedate) + "<br/>" +
                                 "<strong>Crime Code</strong>: " + crime.crimecode + "<br/>"+
                                 "<strong>Location</strong>: " + crime.location + "<br/>" +
                                 "<strong>Description</strong>: " + crime.description + "<br/>"+
                                 "<strong>Weapon</strong>: " + crime.weapon + "<br/>" +
                                 "<strong>District</strong>: " + crime.district + "<br/>" +
                                 "<strong>Neighborhood</strong>: " + crime.neighborhood + "<br/>";

            // From Google Maps Geocoding Tutorial:  https://google-developers.appspot.com/maps/documentation/javascript/examples/geocoding-simple
