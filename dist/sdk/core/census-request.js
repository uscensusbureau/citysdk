(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('promise'), require('jquery'), require('terraformer'), require('terraformer-arcgis-parser')) :
	typeof define === 'function' && define.amd ? define(['promise', 'jquery', 'terraformer', 'terraformer-arcgis-parser'], factory) :
	(global.CensusRequest = factory(global.Promise,global.$,global.Terraformer,global.Terraformer.ArcGIS));
	...
	var CensusRequestUtils = function () {
	  ...
	  createClass(CensusRequestUtils, null, [{
	    ...
	    value: function getGeographyVariables(request) {
	      ...
	      var url = '' + defaultEndpoints$1.censusUrl + request.year + '/' + request.api + '/geography.json';
	...
	var CensusGeoRequest = function () {
	  ...
	  createClass(CensusGeoRequest, null, [{
	    ...
	    value: function supplementalRequest(req, res, featureIndex) {
	      ...
	      var promiseHandler = function promiseHandler(resolve, reject) {
	        var censusSummaryRequest = CensusSummaryRequest.request(suppRequest);
	    ...
	    value: function request(_request) {
	      var promiseHandler = function promiseHandler(resolve, reject) {
	        CensusRequest.request(_request).then(CensusTigerwebRequest.request).then(CensusGeoRequest.handleTigerwebResponse).then(function (response) {
	          return resolve(response);
	        }).catch(function (reason) {
	          return reject(reason);
	        });
	    ...
	    value: function request(_request) {
	      ...
	      var promiseHandler = function promiseHandler(resolve, reject) {
	        var onRequestHasLatLng = function onRequestHasLatLng(request) {
	          CensusRequestUtils.getFipsFromLatLng(request).then(CensusRequestValidator.validateGeoVariables).then(CensusSummaryRequest.request).then(CensusTigerwebRequest.request).then(CensusGeoRequest.handleTigerwebResponse).then(function (response) {
	            return resolve(response);
	          }).catch(function (reason) {
	            return reject(reason);
	          });
	        ...
	        if (!_request.lat && !_request.lng) {
	          ...
	          CensusRequestUtils.getLatLng(_request).then(onRequestHasLatLng).catch(function (reason) {
	            return reject(reason);
	          });
	        } else {
...
//# sourceMappingURL=census-request.js.map