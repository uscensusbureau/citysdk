...
(deftest merge-spooler-stats-test
  (let [args-1 {:sourcePath    ["acs" "acs5"]
    ...
    (test-async-timed
      ...
      (go (let [=args= (chan 1)
                ...
                =E=    (chan 1)]
            (>! =args= args-1)
            ((merge-spooler =args= [pre-cfg-Census-Stats]) =O= =E=)
            (is (= (alt! =O= ([data] data)
                         =E= ([err]  err))
                   ARGS-1-STATS-CLJ))
            (close! =args=)
  ...
  (let [args-1 {:sourcePath    ["acs" "acs5"]
    ...
    (test-async-timed
      ...
      (go (let [=args= (chan 1)
                ...
                =E=    (chan 1)]
            (>! =args= args-1)
            ((merge-spooler *g* =args= [pre-cfg-Census-GeoCLJ]) =O= =E=)
            (is (= (alt! =O= ([data] data)
                         =E= ([err]  err))
                   ARGS-1-GEOS-CLJ))
            (close! =args=)
  ...
  (let [args-2 {:sourcePath    ["acs" "acs5"]
    ...
    (test-async-timed
      ...
      (go (let [=args= (chan 1)
                ...
                f-clj  (fn [data] (prn data))]
            (>! =args= args-3)
            ((merge-spooler *g* =args= [pre-cfg-Census-GeoCLJ pre-cfg-Census-Stats]) =O= =E=)
            (alt! =O= ([data] (f-js data))
                  =E= ([err]  (f-clj err)))
            (close! =args=)
            (close! =O=)
            (close! =E=))))))

;; With transduct<< (all :counties stringified):
; "merge-spooler-merge-test - Heap stats (MB):"
;{:rss 1142.99, :heapTotal 1108.4, :heapUsed 925.6, :external 29.68}
;"merge-spooler-merge-test: Elapsed ms= 29050"
; from cache:
; "merge-spooler-merge-test - Heap stats (MB):"
;{:rss 359.71, :heapTotal 337.71, :heapUsed 292.1, :external 29.59}
;"merge-spooler-merge-test: Elapsed ms= 22633"
